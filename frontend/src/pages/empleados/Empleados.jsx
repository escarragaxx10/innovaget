...
export default function Empleados() {
  ...
  const totalCajeros = empleados.filter(e => e.role === "cajero").length;
  ...
  return (
    <Layout>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}>Gestión de Cajeros</h1>
          <p style={styles.subtitle}>{empleados.length} cajero(s) registrado(s)</p>
        </div>
        <button onClick={abrirModalCrear} style={styles.btnPrimario}>+ Nuevo cajero</button>
      </div>
      ...
      <div style={styles.statsGrid}>
        <div style={styles.statCard}>
          ...
          <div>
            <p style={styles.statNum}>{totalCajeros}</p>
            <p style={styles.statLabel}>Cajeros activos</p>
          </div>
        </div>
        <div style={{ ...styles.statCard, borderColor: "#16a34a40", backgroundColor: "#f0fdf4" }}>
          <div style={{ ...styles.statIconBox, backgroundColor: "#16a34a20", color: "#16a34a" }}>👥</div>
          <div>
            <p style={{ ...styles.statNum, color: "#16a34a" }}>{empleados.length}</p>
            <p style={styles.statLabel}>Total personal</p>
          </div>
        </div>
        <div style={{ ...styles.statCard, borderColor: "#f59e0b40", backgroundColor: "#fffbeb" }}>
          <div style={{ ...styles.statIconBox, backgroundColor: "#f59e0b20", color: "#f59e0b" }}>🏢</div>
          <div>
            <p style={{ ...styles.statNum, color: "#f59e0b" }}>{sucursales.length}</p>
            <p style={styles.statLabel}>Sucursales</p>
          </div>
        </div>
      </div>
      ...
      <div style={styles.searchBar}>
        <span>🔍</span>
        <input type="text" placeholder="Buscar cajero por correo..."
      ...
      <div style={styles.tableCard}>
        {loading ? (
          <div style={styles.emptyBox}>Cargando cajeros...</div>
        ) : empleadosFiltrados.length === 0 ? (
          <div style={styles.emptyBox}>
            <p style={{ fontSize: "48px", margin: "0 0 12px 0" }}>👨‍💼</p>
            <p>No hay cajeros registrados</p>
            <button onClick={abrirModalCrear} style={styles.btnPrimario}>Crear primer cajero</button>
          </div>
        ) : (
          <table style={styles.table}>
            <thead>
              <tr>
                {["Cajero", "Rol", "Sucursal", "Acciones"].map(h => (
              ...
              {empleadosFiltrados.map((emp, i) => {
                ...
                return (
                  <tr key={emp.id} style={{ backgroundColor: i % 2 === 0 ? "#f9fafb" : "#fff" }}>
                    ...
                    <td style={styles.td}>
                      <span style={styles.rolBadge}>🏪 Cajero</span>
                    </td>
                    <td style={styles.td}>
                      {sucursal ? (
          ...
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h2 style={styles.modalTitle}>
                {empleadoEditando ? "Editar cajero" : "Nuevo cajero"}
              </h2>
            ...
            <form onSubmit={handleGuardar} style={styles.modalForm}>
              <div style={styles.fieldGroup}>
                ...
                <input type="email" value={form.email}
                  ...
                  placeholder="cajero@empresa.com" required
                  disabled={!!empleadoEditando}
                  style={{ ...styles.input, backgroundColor: empleadoEditando ? "#f9fafb" : "#fff" }}
              ...
              <div style={styles.infoBox}>
                <p style={styles.infoTitle}>📋 Permisos del cajero:</p>
                <p style={styles.infoItem}>✅ Abrir y cerrar caja</p>
                <p style={styles.infoItem}>✅ Registrar ventas en su sucursal</p>
                <p style={styles.infoItem}>✅ Registrar y buscar clientes</p>
                <p style={styles.infoItem}>❌ No puede ver reportes ni configuración</p>
              </div>
              ...
              <div style={styles.modalFooter}>
                ...
                <button type="submit" disabled={guardando}
                  ...
                  {guardando ? "Guardando..." : empleadoEditando ? "Actualizar cajero" : "Crear cajero"}
                </button>
              </div>
            </form>
          </div>
...
const styles = {
  ...
  statCard: { padding: "20px", borderRadius: "12px", border: "1.5px solid #e5e7eb", backgroundColor: "#f0f9ff", borderColor: "#0369a140", display: "flex", alignItems: "center", gap: "16px" },
  statIconBox: { width: "48px", height: "48px", borderRadius: "10px", backgroundColor: "#0369a120", color: "#0369a1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", flexShrink: 0 },
  statNum: { fontSize: "28px", fontWeight: "700", color: "#0369a1", margin: 0 },
  statLabel: { fontSize: "12px", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.5px", margin: 0 },
  ...
  avatar: { width: "36px", height: "36px", borderRadius: "50%", backgroundColor: "#f0f9ff", color: "#0369a1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", fontWeight: "700", flexShrink: 0, border: "1.5px solid #bae6fd" },
  empEmail: { fontSize: "14px", fontWeight: "600", color: "#111827" },
  empId: { fontSize: "11px", color: "#9ca3af" },
  rolBadge: { padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "700", backgroundColor: "#f0f9ff", color: "#0369a1" },
  ...
  modal: { backgroundColor: "#fff", borderRadius: "16px", padding: "32px", width: "100%", maxWidth: "480px", maxHeight: "90vh", overflowY: "auto" },
  ...
  infoBox: { backgroundColor: "#f0fdf4", borderRadius: "10px", padding: "14px 16px", display: "flex", flexDirection: "column", gap: "6px" },
  infoTitle: { fontSize: "12px", fontWeight: "700", color: "#16a34a", textTransform: "uppercase", letterSpacing: "0.5px", margin: 0 },
  infoItem: { fontSize: "13px", color: "#374151", margin: 0 },
